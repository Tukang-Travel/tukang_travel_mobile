...
import com.google.firebase.codelabs.recommendations.data.Feed
import com.google.firebase.codelabs.recommendations.data.FeedRepository
...
class RecommendationClient(private val context: Context, private val config: Config) {
    private val candidates: MutableMap<Int, Feed> = HashMap()
    ...
    private suspend fun loadCandidateList() {
        return withContext(Dispatchers.IO) {
            val collection = FeedRepository.getInstance(context).getContent()
    ...
    private suspend fun preprocess(selectedFeeds: List<Feed>): IntArray {
        return withContext(Dispatchers.Default) {
            val inputContext = IntArray(config.inputLength)
            for (i in 0 until config.inputLength) {
                if (i < selectedFeeds.size) {
                    val (id) = selectedFeeds[i]
                    inputContext[i] = id
                } else {
                    // Padding input.
    ...
    private suspend fun postprocess(
        outputIds: IntArray, confidences: FloatArray, selectedFeeds: List<Feed>
    ): List<Result> {
        return withContext(Dispatchers.Default) {
            ...
            for (i in outputIds.indices) {
                ...
                if (selectedFeeds.contains(item)) {
    ...
    suspend fun recommend(selectedFeeds: List<Feed>): List<Result> {
        return withContext(Dispatchers.Default) {
            val inputs = arrayOf<Any>(preprocess(selectedFeeds))
            ...
            tflite?.let {
                ...
                postprocess(outputIds, confidences, selectedFeeds)
            } ?: run {
